...
static uint8_t serial_image_buf[SERIAL_IMAGE_SIZE];  // buffer for serial-received images
 ...
 *   ESP32 -> PC : "RESULT:<person_score_pct>:<no_person_score_pct>\n"
...
static int detect_serial_cli_handler(int argc, char *argv[])
{
    ...
    printf("%s: Press Ctrl+C or send 'STOP' to exit.\n", TAG);
    ...
    while (1) {
        ...
        while (bytes_received < total_bytes) {
            int len = uart_read_bytes(uart_num,
                                      &serial_image_buf[bytes_received],
            ...
            if (len > 0) {
                ...
                if (timeout_count >= MAX_TIMEOUTS) {
                    printf("%s: Timeout waiting for image data (got %d/%d bytes)\n",
                           TAG, bytes_received, total_bytes);
                    goto exit_serial;
                }
            }
        }
        ...
        frames_processed++;
        ESP_LOGI(TAG, "Frame #%d processed in %d ms", frames_processed, detect_time);

        // Send back a parseable result line
        // Note: run_inference already prints scores via RespondToDetection,
        // but we add a machine-readable line for the Python script
        char result_msg[64];
...
static esp_console_cmd_t diag_cmds[] = {
    ...
    },
    {
        ...
        .help = "Receive 96x96 grayscale images from PC webcam via serial and run inference",
