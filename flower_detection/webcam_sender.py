    ...
    try:
        while True:
            ...
            while time.time() - start_wait < 10:
                ...
                if "top_flower:" in line:
                    try:
                        # expected format: top_flower:daisy score:85%
                        parts = line.split("score:")
                        flower_part = parts[0].split("top_flower:")[1].strip()
                        score_str = parts[1].split("%")[0].strip()
                        last_flower = flower_part
            ...
            if last_score is not None and last_flower is not None:
                if last_score >= 50:
                    color = (0, 255, 0)
                    label = f"{last_flower.upper()} ({last_score}%)"
                else:
                    color = (0, 0, 255)
                    label = f"Inconnu ({last_score}%)"
